...
export default function Hunt({
  ...
  if (deals.length === 0) return <div className="empty">No beasts in these hunting grounds yet.</div>;
  return (
    <div className="qgrid">
      {deals.map((d) => {
        const n = dealNet(d);
        ...
        const tile = d.image
          ...
          : {
              background: rcol,
              WebkitMaskImage: `url(/art/gi/${beastFor(d.id)}.svg)`,
              maskImage: `url(/art/gi/${beastFor(d.id)}.svg)`,
