...
const params = new URLSearchParams({ keyword: query, sortBy: "2" });
if (maxPrice) params.set("maxPrice", String(maxPrice));
const startUrl = `https://www.mercari.com/search/?${params.toString()}`;
...
async function extractCards(page) {
  return page.$$eval('a[href*="/item/m"]', (anchors) => {
    const out = [];
...
function parseCard(raw) {
  ...
  const titleFromImgAlt = (alt) => (alt ? alt.trim() || undefined : undefined);
  ...
  return { id: `mc_${raw.id}`, title, price, currency: "USD", location: undefined, image: pickImage(raw.src, raw.srcset), url: raw.url, source: "mercari" };
}
